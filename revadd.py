...
print("<header><h1>Reviews</h1></header>")
...
cur = cnx.cursor()
#print("{} {}".format(rest_name,user))
#print("{}".format(com))

sql="insert into comment(c_user,c_restaurant,c_comment) values('%s','%s','%s')"%(user,rest_name,com)
#sql="insert into comment(c_user,c_restaurant,c_comment) values('vignesh_av','mcdonadls','REALLY GOOD FOOD. best place ..loved it too much!!!!')"
try:
	cur.execute(sql)
	cnx.commit()
	print("<br><br>")
	print("<b><i><h3 style='color:maroon;font-family:Times New Roman;'>&nbsp&nbsp&nbsp&nbsp YOUR COMMENT HAS BEEN ADDED SUCCESSFULLY  {}</i></b> </h3><br>".format(user))
except:
	cnx.rollback()
...
message = "From: From TASTY SPOON \nTo: To Person <%s> \nSubject: Thanks\nThank you for adding your review . Please do visit again."%mail
#message = "hi"
try:
